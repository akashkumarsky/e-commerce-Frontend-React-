...
export default function SearchProduct() {
  ...
  useEffect(() => {
    if (keyword) {
      dispatch(searchProduct(keyword)); // Search if keyword exists
    } else {
  ...
  const handleSearch = (e) => {
    const keyword = e.target.value.trim(); // Trim to remove extra spaces
    if (keyword.length > 0) {
      dispatch(searchProduct(keyword));
    } else {
      dispatch(findProducts()); // Fetch all products when input is empty
    }
  };
